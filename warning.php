...
$conn->query("SET time_zone = '+07:00'"); // Việt Nam là GMT+7
...
$ban_end_display = $ban_expired ? '<strong style="color: red; font-weight: bold;">Đã hết hạn</strong>' : date('d/m/Y | H:i:s', strtotime($ban_end));

// Xử lý xóa bản ghi cấm nếu checkbox được chọn và đồng ý
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['agree_terms']) && $_POST['agree_terms'] === '1') {
    ...
    if ($stmt->affected_rows > 0) {
        ...
        $redirect_after = true; // Biến kiểm tra việc chuyển hướng
    } else {
        $message = "Đã xảy ra lỗi. Có thể là do khác múi giờ, hãy thử lại sau 6-12 tiếng.";
        $redirect_after = false; // Không chuyển hướng nếu không có bản ghi cấm
    }
} else {
    $message = "";
    $redirect_after = false; // Không có yêu cầu xóa, không chuyển hướng
}
?>
